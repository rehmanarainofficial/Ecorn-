...
console.log("Total sum of GrandTotal: " + totalSum.toFixed(2)); 
    ...
    const removeItem = async(index) => {
        ...
        dispatch(setGrandCartTotalPrice(GrandCartTotalPrice - cart[index].GrandTotal))
    ...
    return (
        <View style={{ flex: 1, backgroundColor: APPCOLORS.BTN_COLOR, padding: 20, paddingBottom: 0 }}>
            ...
            <Text style={{ color: APPCOLORS.WHITE, fontSize: 25, fontWeight: 'bold', marginTop: 10 }}>Grand Total: Rs {totalSum}</Text>
                    ...
                    return (
                        <View style={{ backgroundColor: APPCOLORS.CLOSETOWHITE, marginTop: 20, borderRadius: 10, padding: 10 }}>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 }}>
                                ...
                                <Text style={{ color: APPCOLORS.BLACK, fontSize: 15 }}>Rs {typeof item.GrandTotal == "number" ? (item.GrandTotal).toFixed(2): item.GrandTotal}</Text>
                            </View>
